    ...
    companion object {
        ...
        fun testExample(test: ContractTest, testBaseUrl: String): Pair<Result, String> {
            ...
            val testLog = buildString {
                testResult.request?.let { req ->
                    appendLine(">> Request")
                    appendLine(req.toLogString())
                }
                testResult.response?.let { resp ->
                    appendLine("<< Response")
                    appendLine(resp.toLogString())
                }
            }.ifBlank { "No Test Logs Found" }

            return result to testLog
        }
        ...
        private fun generateExampleFiles(
            ...
            return discriminatorBasedRequestResponses.filterNot { it.matches(existingDiscriminators) }.map { (request, response, requestDiscriminator, responseDiscriminator, _) ->
        ...
        private fun validateExample(feature: Feature, example: ExampleFromFile): Result {
            return feature.matchResultFlagBased(example.request, example.response, ExampleMismatchMessages, isPartial = false).toResultIfAnyWithCauses()
        }
        ...
        fun getExistingExampleFiles(feature: Feature, scenario: Scenario, examples: List<ExampleFromFile>): List<Pair<ExampleFromFile, Result>> {
            return examples.mapNotNull { example ->
                when (val matchResult = scenario.matches(example.request, example.response, ExampleMismatchMessages, feature.flagsBased)) {
        ...
        fun transformExistingExamples(contractFile: File, overlayFile: File?, examplesDir: File) {
            ...
            examples.forEach { example ->
                ...
                val scenario = feature.matchResultFlagBased(example.request, example.response, ExampleMismatchMessages, isPartial = false)
        ...
        private fun writeToExampleFile(scenarioStub: ScenarioStub, contractFile: File): File {
            ...
            val uniqueNameForApiOperation =
                uniqueNameForApiOperation(scenarioStub.request, "", scenarioStub.response.status, examplesDir)
        ...
        private fun validatePartialFailures(feature: Feature, exampleFromFile: ExampleFromFile): List<Result> {
            return feature.matchResultFlagBased(exampleFromFile.request, exampleFromFile.response, ExampleMismatchMessages, isPartial = false).toResultPartialFailures()
        }
